...
<div class="wz">
	您当前所在的位置：<a onclick="return loadPage(this);" href="index.html" class="news">首页</a>—<a onclick="return loadPage(this);" href="hotel.html" class="news">
		<c:out value="${hAddress.name }" default="桂林酒店"/>
		</a>
</div>
<%--
<div class="lv_sos">
...
<div class="same_list_back">
		...
		<ul>
			<c:forEach items="${hotels }" var="h">
				<li><span><strong style="font-size: 14px; color: #F60;">${h.name }</strong><br />周边景点：${h.attractons }<br />
					...
					介绍：<html:out html="${h.remark }" length="300"/>  <br /> <a href="hotel/view/${h.id }.html" class="xq">[查看详情]</a>
					<a href="hotel/view/${h.id }.html#room"><img src="image/yd.gif" align="absmiddle" /></a></span><a href="hotel/view/${h.id }.html"><img
	...
	<%--页码 开始 --%>
	<div class="pag">
		<span class="disabled">&lt; </span><a href="search/hotel.html?q=${q }&page=1">首 页</a><a
			href="search/hotel.html?q=${q }&page=${page.prev }">上一页</a>
		<c:forEach items="${page.pages }" var="p">
			<c:choose>
				...
				<c:otherwise>
					<a href="search/hotel.html?q=${q }&page=${p }">${p }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<c:if test="${page.page + 6 < page.last }">
			<c:out value="..."></c:out>
		</c:if>
		<a href="search/hotel.html?q=${q }&page=${page.next }">下一页</a><a href="search/hotel.html?q=${q }&page=${page.last }">尾页</a>
	</div>
	<%--页码 结束 --%>
</div>