		...
		<ul>
			<c:forEach items="${tours }" var="t">
				<li><span class="jg">${t.privilege }元起</span><a
					...
					class="font_hs">&nbsp;<html:out html="${t.plan }" length="30" /></span></li>
			</c:forEach>
		</ul>
	</div>
	<%--页码 开始 --%>
	<div class="pag">
		<span class="disabled">&lt; </span><a href="search/tour.html?q=${q }&page=1">首 页</a><a
			href="search/tour.html?q=${q }&page=${page.prev }">上一页</a>
		<c:forEach items="${page.pages }" var="p">
			<c:choose>
				...
				<c:otherwise>
					<a href="search/tour.html?q=${q }&page=${p }">${p }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<a href="search/tour.html?q=${q }&page=${page.next }">下一页</a><a href="search/tour.html?q=${q }&page=${page.last }">尾页</a>
	</div>
	<%--页码 结束 --%>
</div>