...
<div class="same_list_back">
	...
	<!-- 页码 开始 -->
	<div class="pag">
		<span class="disabled">&lt; </span><a href="search/hotel?${params }&page=1">首 页</a><a
			href="search/hotel?${params }&page=${page.prev }">上一页</a>
		<c:forEach items="${page.pages }" var="p">
			<c:choose>
				...
				<c:otherwise>
					<a href="search/hotel?${params }&page=${p }">${p }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<a href="search/hotel?${params }&page=${page.next }">下一页</a><a href="search/hotel?${params }&page=${page.last }">尾页</a>
	</div>
	<!-- 页码 结束 -->
</div>