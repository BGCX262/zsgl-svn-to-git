...
@RooJavaBean
@RooToString
@RooJpaActiveRecord(finders = { "findPricesByDateBetween", "findPricesByDateGreaterThan", "findPricesByDateLessThan" })
public class Price {
	...
	public static TypedQuery<Price> findPricesByDateBetween(Date minDate, Date maxDate) {
        ...
        EntityManager em = Price.entityManager();
        TypedQuery<Price> q = em.createQuery("SELECT o FROM Price AS o WHERE o.date BETWEEN :minDate AND :maxDate", Price.class);
        q.setParameter("minDate", minDate);
        q.setParameter("maxDate", maxDate);
        return q;
    }

	public static TypedQuery<Price> findPricesByDateGreaterThan(Date date) {
        if (date == null) throw new IllegalArgumentException("The date argument is required");
        EntityManager em = Price.entityManager();
        TypedQuery<Price> q = em.createQuery("SELECT o FROM Price AS o WHERE o.date > :date", Price.class);
        q.setParameter("date", date);
        return q;
    }

	public static TypedQuery<Price> findPricesByDateLessThan(Date date) {
        if (date == null) throw new IllegalArgumentException("The date argument is required");
        EntityManager em = Price.entityManager();
        TypedQuery<Price> q = em.createQuery("SELECT o FROM Price AS o WHERE o.date < :date", Price.class);
        q.setParameter("date", date);
